# analyzer_pro.py - Профессиональный анализатор переписок для SaaS-ОКК
import openai
import json
import os
import logging
from typing import List, Dict, Any
from datetime import datetime, timedelta
from dotenv import load_dotenv

load_dotenv()

logger = logging.getLogger(__name__)

# Настройки
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-4o-mini")

def analyze_chat_pro(messages: List[Dict[str, Any]], metadata: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    Профессиональный анализ чата для ОКК (Отдел Контроля Качества)
    Возвращает структурированный отчет в формате таблицы
    """
    try:
        # Проверка API ключа
        if not OPENAI_API_KEY:
            return create_pro_error_response("OpenAI API key not configured")
        
        logger.info(f"🔍 Профессиональный анализ: {len(messages)} сообщений")
        
        # Форматируем диалог
        formatted_chat = format_chat_for_pro_analysis(messages)
        
        # Промпт эксперта ОКК
        system_prompt = """Ты — аналитическая языковая модель, ядро SaaS-продукта в сфере ИИ-ОКК (отдел контроля качества коммуникаций).

Твоя задача — анализировать текстовые переписки между менеджером и клиентом в CRM-системах (мессенджеры и чаты) и выдавать структурированный, стандартизированный аналитический отчёт.

Переписки могут быть из любой ниши. Ты не используешь отраслевые знания, а опираешься на универсальные законы деловой коммуникации, продаж и консультаций в чатах.

Ты анализируешь не «качество текста», а эффективность коммуникации с точки зрения бизнеса.

--------------------------------
РОЛЬ И ПРИНЦИПЫ МЫШЛЕНИЯ
--------------------------------

Ты — эксперт мирового уровня по аналитике переписок и контролю качества коммуникаций.
За карьеру ты проанализировал десятки миллионов чатов и знаешь все типовые сценарии продаж, консультаций и потерь клиентов в текстовых каналах.

Твои принципы:
- Ты отделяешь ФАКТ от ИНТЕРПРЕТАЦИИ.
- Ты не даёшь общих или очевидных советов.
- Ты не пишешь «воды» и абстрактных рекомендаций.
- Ты ориентирован на масштабируемость анализа.
- Ты всегда думаешь с позиции собственника бизнеса.

Если факт невозможно однозначно определить из переписки — укажи это явно.

--------------------------------
ЕДИНИЦА АНАЛИЗА
--------------------------------

Единица анализа — один завершённый диалог (чат) между менеджером и клиентом.

--------------------------------
СЛОИ АНАЛИЗА (ОБЯЗАТЕЛЬНЫ)
--------------------------------

Ты ОБЯЗАН последовательно проанализировать каждый чат по следующим слоям.
Ни один слой нельзя пропускать.

--------------------------------
L0. Контекст чата (метаданные)
--------------------------------
Если данные отсутствуют — укажи «не указано».

- Менеджер
- Канал коммуникации
- Дата и время диалога
- Этап воронки
- Связь с CRM (сделка / контакт)
- Среднее время ответа менеджера (если возможно определить)

--------------------------------
L1. Формальные действия менеджера
--------------------------------
Ответ строго в формате: Да / Нет / Неопределимо

- Было ли приветствие
- Было ли прощание
- Был ли вежливый и корректный тон
- Был ли конфликт или негатив
- Отвечал ли менеджер по сути вопросов клиента
- Было ли предложено следующее действие (CTA)

--------------------------------
L2. Работа с потребностями
--------------------------------
Фиксируй только факты, а не намерения.

- Были ли вопросы к клиенту
- Уточнялись ли параметры запроса
- Были ли зафиксированы потребности клиента
- Есть ли движение от запроса клиента к предложению решения

--------------------------------
L3. Работа с возражениями
--------------------------------
Если возражений не было — укажи это явно.

- Были ли возражения со стороны клиента
- Была ли попытка отработки возражений
- Были ли возражения закрыты
- Тип возражений (если можно определить: цена / срок / сомнение / другое)

--------------------------------
L4. Интерес и готовность клиента
--------------------------------

- Проявляет ли клиент интерес
- Готов ли клиент продолжать диалог
- Запрашивал ли клиент дополнительную информацию
- Согласился ли клиент на следующий шаг

--------------------------------
L5. Результат диалога
--------------------------------

- Диалог завершён или нет
- Есть ли договорённость
- Зафиксирован ли следующий контакт
- Итоговый статус: продолжение / пауза / отказ / неопределено

--------------------------------
L6. Рекомендации по чату
--------------------------------

Дай рекомендации, соблюдая правила:
- Рекомендации должны быть предметными и привязанными к конкретным моментам диалога.
- Запрещены общие формулировки («нужно лучше», «стоит больше»).
- Каждая рекомендация должна быть применима к аналогичным чатам.
- Если всё сделано корректно — укажи это прямо.

Формат:
- Что сделано хорошо
- Что было упущено
- Что можно улучшить в этом чате

--------------------------------
ФОРМАТ ВЫВОДА
--------------------------------

Верни результат в виде JSON со следующими ключами:

{
    "l0_context": {
        "manager": "имя или id менеджера",
        "channel": "whatsapp/telegram/viber/etc",
        "datetime": "дата и время",
        "funnel_stage": "холод/тепло/горячий",
        "crm_link": "ссылка на сделку/контакт",
        "avg_response_time": "среднее время ответа в минутах"
    },
    "l1_formal_actions": {
        "greeting": "Да/Нет/Неопределимо",
        "farewell": "Да/Нет/Неопределимо",
        "polite_tone": "Да/Нет/Неопределимо",
        "conflict": "Да/Нет/Неопределимо",
        "answered_substantively": "Да/Нет/Неопределимо",
        "cta_proposed": "Да/Нет/Неопределимо"
    },
    "l2_needs_work": {
        "questions_to_client": "Да/Нет/Неопределимо",
        "request_parameters_clarified": "Да/Нет/Неопределимо",
        "needs_recorded": "Да/Нет/Неопределимо",
        "movement_to_solution": "Да/Нет/Неопределимо",
        "needs_explanation": "краткое пояснение (1-2 предложения) что именно выявили или не смогли выявить"
    },
    "l3_objections_work": {
        "objections_exist": "Да/Нет/Неопределимо",
        "objection_handling_attempt": "Да/Нет/Неопределимо",
        "objections_closed": "Да/Нет/Неопределимо",
        "objection_type": "цена/срок/сомнение/другое/нет",
        "objection_explanation": "краткое пояснение (1-2 предложения) какие возражения были и как отработаны"
    },
    "l4_client_interest": {
        "client_shows_interest": "Да/Нет/Неопределимо",
        "client_willing_to_continue": "Да/Нет/Неопределимо",
        "client_requested_info": "Да/Нет/Неопределимо",
        "client_agreed_next_step": "Да/Нет/Неопределимо",
        "interest_explanation": "краткое пояснение (1-2 предложения) как именно проявляется интерес клиента"
    },
    "l5_dialog_result": {
        "dialog_completed": "Да/Нет/Неопределимо",
        "agreement_exists": "Да/Нет/Неопределимо",
        "next_contact_scheduled": "Да/Нет/Неопределимо",
        "final_status": "продолжение/пауза/отказ/неопределено"
    },
    "l6_recommendations": {
        "done_well": ["конкретная сильная сторона 1", "сильная сторона 2"],
        "missed_opportunities": ["что упущено 1", "что упущено 2"],
        "improvements": ["конкретное улучшение 1", "улучшение 2"]
    },
    "chat_preview": "первые 500 символов диалога для отображения в таблице",
    "summary_score": 0-100
}

Без воды и лирики.
Используй профессиональные термины только при необходимости и всегда кратко поясняй их.
Не придумывай факты.
Не смягчай выводы.
Анализируй как продукт, а не как консультант."""
        
        # Подготовка контекста метаданных
        meta_info = ""
        if metadata:
            meta_info = f"\nМетаданные чата:\n"
            for key, value in metadata.items():
                meta_info += f"- {key}: {value}\n"
        
        # Вычисляем среднее время ответа
        response_time = calculate_average_response_time(messages)
        
        # Добавляем вычисленное время в промпт
        user_prompt = f"""{formatted_chat}

{meta_info}

Вычисленные метрики:
- Среднее время ответа менеджера: {response_time}
- Всего сообщений: {len(messages)}
- Сообщений менеджера: {len([m for m in messages if m.get('role') == 'manager'])}
- Сообщений клиента: {len([m for m in messages if m.get('role') == 'client'])}

Проанализируй этот диалог по всем слоям (L0-L6) строго в указанном формате JSON."""
        
        # API запрос - ИСПРАВЛЕНО!
        client = openai.OpenAI(
            api_key=OPENAI_API_KEY,
            timeout=60.0
        )
        
        response = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_prompt}
            ],
            temperature=0.1,
            max_tokens=2000,
            response_format={"type": "json_object"}
        )
        
        # Парсим результат
        result_text = response.choices[0].message.content
        result = json.loads(result_text)
        
        # Валидация и обогащение результата
        validated_result = validate_and_enrich_pro_analysis(result, messages, metadata)
        
        logger.info(f"✅ Профессиональный анализ завершен. Счет: {validated_result.get('summary_score', 0)}")
        
        return validated_result
        
    except json.JSONDecodeError as e:
        logger.error(f"❌ JSON parsing error: {e}")
        return create_pro_error_response(f"JSON parsing error: {str(e)}")
    except openai.APIConnectionError as e:
        logger.error(f"❌ OpenAI connection error: {e}")
        return create_pro_error_response(f"OpenAI connection error: {str(e)}")
    except Exception as e:
        logger.error(f"❌ Professional analysis error: {e}")
        return create_pro_error_response(f"Analysis error: {str(e)}")

def format_chat_for_pro_analysis(messages: List[Dict[str, Any]]) -> str:
    """Форматирует диалог для профессионального анализа"""
    lines = [
        "=" * 80,
        "ДИАЛОГ МЕНЕДЖЕРА С КЛИЕНТОМ (ПРОФЕССИОНАЛЬНЫЙ АНАЛИЗ ОКК)",
        "=" * 80,
        ""
    ]
    
    for i, msg in enumerate(messages, 1):
        role = "👨‍💼 МЕНЕДЖЕР" if msg.get("role") == "manager" else "👤 КЛИЕНТ"
        text = msg.get("text", "")
        timestamp = msg.get("timestamp", "")
        
        time_str = f" [{timestamp}]" if timestamp else ""
        lines.append(f"{i}. {role}{time_str}:")
        lines.append(f"   \"{text}\"")
        lines.append("")
    
    lines.append("=" * 80)
    
    return "\n".join(lines)

def calculate_average_response_time(messages: List[Dict[str, Any]]) -> str:
    """Вычисляет среднее время ответа менеджера"""
    try:
        # Фильтруем сообщения менеджера с таймстемпами
        manager_messages = []
        for msg in messages:
            if msg.get("role") == "manager" and msg.get("timestamp"):
                try:
                    if isinstance(msg["timestamp"], str):
                        dt = datetime.fromisoformat(msg["timestamp"].replace('Z', '+00:00'))
                    elif isinstance(msg["timestamp"], (int, float)):
                        dt = datetime.fromtimestamp(msg["timestamp"])
                    else:
                        continue
                    manager_messages.append((dt, msg.get("text", "")))
                except:
                    continue
        
        if len(manager_messages) < 2:
            return "недостаточно данных"
        
        # Сортируем по времени
        manager_messages.sort(key=lambda x: x[0])
        
        # Находим время ответов после сообщений клиента
        response_times = []
        last_client_message_time = None
        
        for msg in messages:
            if msg.get("role") == "client" and msg.get("timestamp"):
                try:
                    if isinstance(msg["timestamp"], str):
                        dt = datetime.fromisoformat(msg["timestamp"].replace('Z', '+00:00'))
                    elif isinstance(msg["timestamp"], (int, float)):
                        dt = datetime.fromtimestamp(msg["timestamp"])
                    else:
                        continue
                    last_client_message_time = dt
                except:
                    continue
            elif msg.get("role") == "manager" and msg.get("timestamp") and last_client_message_time:
                try:
                    if isinstance(msg["timestamp"], str):
                        dt = datetime.fromisoformat(msg["timestamp"].replace('Z', '+00:00'))
                    elif isinstance(msg["timestamp"], (int, float)):
                        dt = datetime.fromtimestamp(msg["timestamp"])
                    else:
                        continue
                    
                    response_time = (dt - last_client_message_time).total_seconds() / 60  # в минутах
                    if response_time > 0:
                        response_times.append(response_time)
                    last_client_message_time = None
                except:
                    continue
        
        if response_times:
            avg_time = sum(response_times) / len(response_times)
            return f"{avg_time:.1f} минут"
        else:
            return "недостаточно данных"
            
    except Exception as e:
        logger.debug(f"Response time calculation error: {e}")
        return "недостаточно данных"

def validate_and_enrich_pro_analysis(result: Dict[str, Any], messages: List[Dict], metadata: Dict = None) -> Dict[str, Any]:
    """Валидация и обогащение профессионального анализа"""
    
    # Убедимся, что все слои присутствуют
    required_layers = [
        "l0_context", "l1_formal_actions", "l2_needs_work", 
        "l3_objections_work", "l4_client_interest", 
        "l5_dialog_result", "l6_recommendations"
    ]
    
    for layer in required_layers:
        if layer not in result:
            result[layer] = {}
    
    # Заполняем контекст если есть метаданные
    if metadata and "l0_context" in result:
        for key, value in metadata.items():
            if key in ["manager", "channel", "datetime", "funnel_stage", "crm_link"]:
                result["l0_context"][key] = value
    
    # Добавляем вычисленные метаданные
    if "l0_context" in result:
        if "avg_response_time" not in result["l0_context"]:
            result["l0_context"]["avg_response_time"] = calculate_average_response_time(messages)
        
        # Добавляем базовую статистику
        result["l0_context"]["message_count"] = len(messages)
        result["l0_context"]["manager_message_count"] = len([m for m in messages if m.get("role") == "manager"])
        result["l0_context"]["client_message_count"] = len([m for m in messages if m.get("role") == "client"])
    
    # Вычисляем summary_score если его нет
    if "summary_score" not in result:
        result["summary_score"] = calculate_pro_score(result)
    
    # Добавляем временную метку анализа
    result["analysis_timestamp"] = datetime.now().isoformat()
    result["model_used"] = OPENAI_MODEL
    
    # Добавляем ID чата если есть
    if metadata and "chat_id" in metadata:
        result["chat_id"] = metadata["chat_id"]
    
    # Добавляем preview чата (первые 500 символов)
    if "chat_preview" not in result:
        chat_text = ""
        for msg in messages[:10]:  # Первые 10 сообщений
            role = "manager" if msg.get("role") == "manager" else "client"
            text = msg.get("text", "")
            timestamp = msg.get("timestamp", "")
            chat_text += f"{timestamp} - {role}: {text}\n"
        result["chat_preview"] = chat_text[:500]
    
    return result

def calculate_pro_score(analysis: Dict[str, Any]) -> int:
    """Вычисляет общий счет на основе анализа"""
    score = 50  # Базовый счет
    
    try:
        # L1: Формальные действия (макс +20)
        l1 = analysis.get("l1_formal_actions", {})
        if l1.get("greeting") == "Да":
            score += 3
        if l1.get("farewell") == "Да":
            score += 3
        if l1.get("polite_tone") == "Да":
            score += 4
        if l1.get("answered_substantively") == "Да":
            score += 5
        if l1.get("cta_proposed") == "Да":
            score += 5
        
        # L2: Работа с потребностями (макс +15)
        l2 = analysis.get("l2_needs_work", {})
        if l2.get("questions_to_client") == "Да":
            score += 4
        if l2.get("request_parameters_clarified") == "Да":
            score += 4
        if l2.get("movement_to_solution") == "Да":
            score += 7
        
        # L4: Интерес клиента (макс +10)
        l4 = analysis.get("l4_client_interest", {})
        if l4.get("client_shows_interest") == "Да":
            score += 5
        if l4.get("client_agreed_next_step") == "Да":
            score += 5
        
        # L5: Результат (макс +15)
        l5 = analysis.get("l5_dialog_result", {})
        if l5.get("agreement_exists") == "Да":
            score += 7
        if l5.get("next_contact_scheduled") == "Да":
            score += 8
        
        # Штрафы
        if l1.get("conflict") == "Да":
            score -= 10
        
        # Ограничиваем от 0 до 100
        score = max(0, min(100, score))
        
    except:
        score = 50  # При ошибке - нейтральный счет
    
    return score

def create_pro_error_response(error_message: str) -> Dict[str, Any]:
    """Создает ответ об ошибке для профессионального анализа"""
    return {
        "error": True,
        "error_message": error_message,
        "l0_context": {
            "manager": "не указано",
            "channel": "не указано",
            "datetime": "не указано",
            "funnel_stage": "не указано",
            "crm_link": "не указано",
            "avg_response_time": "не указано"
        },
        "l1_formal_actions": {
            "greeting": "Неопределимо",
            "farewell": "Неопределимо",
            "polite_tone": "Неопределимо",
            "conflict": "Неопределимо",
            "answered_substantively": "Неопределимо",
            "cta_proposed": "Неопределимо"
        },
        "l2_needs_work": {
            "questions_to_client": "Неопределимо",
            "request_parameters_clarified": "Неопределимо",
            "needs_recorded": "Неопределимо",
            "movement_to_solution": "Неопределимо"
        },
        "l3_objections_work": {
            "objections_exist": "Неопределимо",
            "objection_handling_attempt": "Неопределимо",
            "objections_closed": "Неопределимо",
            "objection_type": "нет"
        },
        "l4_client_interest": {
            "client_shows_interest": "Неопределимо",
            "client_willing_to_continue": "Неопределимо",
            "client_requested_info": "Неопределимо",
            "client_agreed_next_step": "Неопределимо"
        },
        "l5_dialog_result": {
            "dialog_completed": "Неопределимо",
            "agreement_exists": "Неопределимо",
            "next_contact_scheduled": "Неопределимо",
            "final_status": "неопределено"
        },
        "l6_recommendations": {
            "done_well": [],
            "missed_opportunities": ["Ошибка анализа: " + error_message],
            "improvements": ["Проверить подключение к ИИ"]
        },
        "summary_score": 0,
        "analysis_timestamp": datetime.now().isoformat()
    }

def convert_to_table_row(analysis: Dict[str, Any], chat_id: str = "") -> Dict[str, Any]:
    """Конвертирует анализ в строку таблицы в формате ОКК (sample.xlsx)"""
    
    if analysis.get("error"):
        return {
            "Ответственный менеджер": "Ошибка",
            "Менеджер в диалоге": analysis.get("error_message", "Unknown error"),
            "Тэг": "Ошибка анализа",
            "Дата": "",
            "Пояснение": analysis.get("error_message", ""),
            "Среднее время ответа (минуты)": "",
            "Воронка -> этап": "",
            "Ссылка на сделку": "",
            "Анализируемая коммуникация": "",
            "1. Поздоровался?": "",
            "2. Был конфликт?": "",
            "3.1 Выявили потребности?": "",
            "3.2 Пояснение": "",
            "4. Были возражения?": "",
            "5. Была попытка закрыть возражения?": "",
            "6.1 Возражение закрыто?": "",
            "6.2 Пояснение": "",
            "7.1 Интерес клиента к сделке": "",
            "7.2 Пояснение": "",
            "Рекомендация для менеджера": analysis.get("error_message", "")
        }
    
    row = {}
    
    # L0: Контекст
    l0 = analysis.get("l0_context", {})
    row["Ответственный менеджер"] = l0.get("manager", "не указано")
    row["Менеджер в диалоге"] = l0.get("channel", "не указано")
    row["Дата"] = l0.get("datetime", "не указано")
    row["Среднее время ответа (минуты)"] = l0.get("avg_response_time", "не указано")
    row["Воронка -> этап"] = l0.get("funnel_stage", "не указано")
    row["Ссылка на сделку"] = l0.get("crm_link", "не указано")
    
    # Тэг - определяем на основе анализа
    row["Тэг"] = generate_tag(analysis)
    
    # Пояснение - общее объяснение
    row["Пояснение"] = generate_general_explanation(analysis)
    
    # Анализируемая коммуникация
    row["Анализируемая коммуникация"] = analysis.get("chat_preview", "")
    
    # L1: Формальные действия
    l1 = analysis.get("l1_formal_actions", {})
    row["1. Поздоровался?"] = map_yes_no(l1.get("greeting", "Неопределимо"))
    row["2. Был конфликт?"] = map_yes_no(l1.get("conflict", "Неопределимо"))
    
    # L2: Работа с потребностями
    l2 = analysis.get("l2_needs_work", {})
    row["3.1 Выявили потребности?"] = map_yes_no(l2.get("needs_recorded", "Неопределимо"))
    row["3.2 Пояснение"] = l2.get("needs_explanation", "")
    
    # L3: Работа с возражениями
    l3 = analysis.get("l3_objections_work", {})
    row["4. Были возражения?"] = map_yes_no(l3.get("objections_exist", "Неопределимо"))
    row["5. Была попытка закрыть возражения?"] = map_yes_no(l3.get("objection_handling_attempt", "Неопределимо"))
    row["6.1 Возражение закрыто?"] = map_yes_no(l3.get("objections_closed", "Неопределимо"))
    row["6.2 Пояснение"] = l3.get("objection_explanation", "")
    
    # L4: Интерес клиента
    l4 = analysis.get("l4_client_interest", {})
    row["7.1 Интерес клиента к сделке"] = map_yes_no(l4.get("client_shows_interest", "Неопределимо"))
    row["7.2 Пояснение"] = l4.get("interest_explanation", "")
    
    # L6: Рекомендации
    l6 = analysis.get("l6_recommendations", {})
    recommendations = []
    recommendations.extend(l6.get("done_well", []))
    recommendations.extend(l6.get("improvements", []))
    row["Рекомендация для менеджера"] = " ".join(recommendations) if recommendations else "Нет рекомендаций"
    
    return row

def map_yes_no(value: str) -> str:
    """Преобразует Да/Нет/Неопределимо в понятный формат"""
    mapping = {
        "Да": "✅ Да",
        "Нет": "❌ Нет", 
        "Неопределимо": "❓ Нет данных",
        "Yes": "✅ Да",
        "No": "❌ Нет",
        "Undefined": "❓ Нет данных"
    }
    return mapping.get(value, value)

def map_objection_type(value: str) -> str:
    """Преобразует тип возражений"""
    mapping = {
        "цена": "💰 Цена",
        "срок": "⏰ Срок",
        "сомнение": "🤔 Сомнение", 
        "другое": "📌 Другое",
        "нет": "➖ Нет возражений",
        "price": "💰 Цена",
        "timing": "⏰ Срок",
        "doubt": "🤔 Сомнение",
        "other": "📌 Другое",
        "none": "➖ Нет возражений"
    }
    return mapping.get(value, value)

def map_final_status(value: str) -> str:
    """Преобразует итоговый статус"""
    mapping = {
        "продолжение": "🔄 Продолжение",
        "пауза": "⏸️ Пауза", 
        "отказ": "❌ Отказ",
        "неопределено": "❓ Не определено",
        "continuation": "🔄 Продолжение",
        "pause": "⏸️ Пауза",
        "rejection": "❌ Отказ",
        "undefined": "❓ Не определено"
    }
    return mapping.get(value, value)

def generate_tag(analysis: Dict[str, Any]) -> str:
    """Генерирует тэг на основе анализа"""
    l1 = analysis.get("l1_formal_actions", {})
    l3 = analysis.get("l3_objections_work", {})
    l4 = analysis.get("l4_client_interest", {})
    l5 = analysis.get("l5_dialog_result", {})
    
    # Проверяем на проблемы
    if l1.get("greeting") != "Да" and l1.get("greeting") != "Yes":
        return "Нет приветствия"
    
    if l1.get("conflict") == "Да" or l1.get("conflict") == "Yes":
        return "Конфликт"
    
    if l3.get("objections_exist") == "Да" or l3.get("objections_exist") == "Yes":
        if l3.get("objections_closed") != "Да" and l3.get("objections_closed") != "Yes":
            return "Возражение не закрыто"
    
    if l4.get("client_shows_interest") != "Да" and l4.get("client_shows_interest") != "Yes":
        return "Клиент не проявляет интерес"
    
    if l5.get("dialog_completed") != "Да" and l5.get("dialog_completed") != "Yes":
        return "Диалог не завершен"
    
    if l5.get("agreement_exists") == "Да" or l5.get("agreement_exists") == "Yes":
        return "Есть договоренность"
    
    return "Нормальный диалог"

def generate_general_explanation(analysis: Dict[str, Any]) -> str:
    """Генерирует общее пояснение на основе анализа"""
    l6 = analysis.get("l6_recommendations", {})
    l1 = analysis.get("l1_formal_actions", {})
    l4 = analysis.get("l4_client_interest", {})
    
    explanation_parts = []
    
    # Добавляем ключевые observations
    if l1.get("conflict") == "Да" or l1.get("conflict") == "Yes":
        explanation_parts.append("В диалоге присутствовал конфликт.")
    
    if l4.get("client_shows_interest") == "Да" or l4.get("client_shows_interest") == "Yes":
        explanation_parts.append("Клиент проявил интерес к предложению.")
    
    if l4.get("client_shows_interest") != "Да" and l4.get("client_shows_interest") != "Yes":
        explanation_parts.append("Клиент не проявил явного интереса к сделке.")
    
    # Добавляем рекомендации
    improvements = l6.get("improvements", [])
    if improvements:
        explanation_parts.append(" ".join(improvements[:2]))  # Первые 2 улучшения
    
    if not explanation_parts:
        return "Диалог прошел в штатном режиме."
    
    return " ".join(explanation_parts)

def calculate_efficiency(row: Dict[str, Any]) -> str:
    """Рассчитывает эффективность на основе ключевых метрик"""
    try:
        score = 0
        total = 0
        
        # Проверяем ключевые метрики
        metrics = {
            "Приветствие": 1,
            "Прощание": 1,
            "Вежливый тон": 2,
            "Ответ по сути": 2,
            "Предложено CTA": 2,
            "Вопросы клиенту": 1,
            "Движение к решению": 2,
            "Клиент согласился на шаг": 2,
            "Договоренность есть": 2
        }
        
        for metric, weight in metrics.items():
            if metric in row:
                value = row[metric]
                if "✅" in value:
                    score += weight
                total += weight
        
        if total == 0:
            return "❓ Нет данных"
        
        efficiency = (score / total) * 100
        
        if efficiency >= 80:
            return f"🎯 Отлично ({efficiency:.0f}%)"
        elif efficiency >= 60:
            return f"👍 Хорошо ({efficiency:.0f}%)"
        elif efficiency >= 40:
            return f"👌 Удовлетворительно ({efficiency:.0f}%)"
        else:
            return f"⚠️ Нужно улучшить ({efficiency:.0f}%)"
            
    except:
        return "❓ Ошибка расчета"

def get_score_category(score: int) -> str:
    """Определяет категорию по счету"""
    if score >= 80:
        return "🎯 Отлично"
    elif score >= 60:
        return "👍 Хорошо"
    elif score >= 40:
        return "👌 Удовлетворительно"
    else:
        return "⚠️ Требует улучшения"

def get_table_headers() -> List[str]:
    """Возвращает заголовки для таблицы в формате ОКК (sample.xlsx)"""
    return [
        "Ответственный менеджер",
        "Менеджер в диалоге",
        "Тэг",
        "Дата",
        "Пояснение",
        "Среднее время ответа (минуты)",
        "Воронка -> этап",
        "Ссылка на сделку",
        "Анализируемая коммуникация",
        "1. Поздоровался?",
        "2. Был конфликт?",
        "3.1 Выявили потребности?",
        "3.2 Пояснение",
        "4. Были возражения?",
        "5. Была попытка закрыть возражения?",
        "6.1 Возражение закрыто?",
        "6.2 Пояснение",
        "7.1 Интерес клиента к сделке",
        "7.2 Пояснение",
        "Рекомендация для менеджера"
    ]

def print_pro_analysis_table(analyses: List[Dict[str, Any]]):
    """Печатает таблицу с анализами"""
    if not analyses:
        print("❌ Нет данных для отображения")
        return
    
    headers = get_table_headers()
    
    # Максимальные ширины столбцов
    col_widths = {}
    for header in headers:
        col_widths[header] = len(header)
    
    # Обновляем ширины на основе данных
    for analysis in analyses:
        row = convert_to_table_row(analysis)
        for header in headers:
            value = str(row.get(header, ""))
            col_widths[header] = max(col_widths[header], len(value))
    
    # Ограничиваем ширину для длинных полей
    max_width = 25
    long_columns = ["Сделано хорошо", "Упущенные возможности", "Улучшения"]
    for header in long_columns:
        col_widths[header] = min(col_widths[header], max_width)
    
    # Печатаем заголовки
    print("\n" + "=" * 180)
    print("📊 ТАБЛИЦА АНАЛИЗА ЧАТОВ (ОКК) - РУССКИЕ НАЗВАНИЯ")
    print("=" * 180)
    
    # Печатаем заголовки (первые 15)
    print("Первые 15 колонок:")
    header_line = ""
    for header in headers[:15]:
        width = col_widths[header]
        header_line += f"{header:<{width}} | "
    print(header_line)
    print("-" * 180)
    
    # Печатаем данные (первые 15 колонок)
    for i, analysis in enumerate(analyses, 1):
        row = convert_to_table_row(analysis)
        row_line = ""
        for header in headers[:15]:
            value = str(row.get(header, ""))
            width = col_widths[header]
            
            # Обрезаем длинные значения
            if len(value) > width:
                value = value[:width-3] + "..."
            
            row_line += f"{value:<{width}} | "
        
        print(f"{i:3}. {row_line}")
    
    print("=" * 180)

def export_to_csv(analyses: List[Dict[str, Any]], filename: str = "okc_analysis.csv"):
    """Экспортирует анализы в CSV файл с русскими заголовками"""
    import csv
    
    headers = get_table_headers()
    
    with open(filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=headers, delimiter=';')
        writer.writeheader()
        
        for analysis in analyses:
            row = convert_to_table_row(analysis)
            writer.writerow(row)
    
    print(f"✅ Данные экспортированы в {filename} ({len(analyses)} строк)")
    print(f"📊 Колонки: {len(headers)}")
    return filename

# Тестовый пример
if __name__ == "__main__":
    print("🧪 Тестируем профессиональный анализатор ОКК с исправлением ошибки")
    print("=" * 60)
    
    test_chat = [
        {"role": "client", "text": "Здравствуйте, интересует доставка пиццы", "timestamp": "2024-01-20T18:05:00"},
        {"role": "manager", "text": "Привет! Конечно, какая пицца вас интересует?", "timestamp": "2024-01-20T18:06:00"},
        {"role": "client", "text": "Маргарита 30см", "timestamp": "2024-01-20T18:07:00"},
        {"role": "manager", "text": "Отлично! Маргарита 30см - 590 руб. Доставка 150 руб. Итого 740 руб. Адрес доставки?", "timestamp": "2024-01-20T18:08:00"},
        {"role": "client", "text": "Дорого. Есть скидки?", "timestamp": "2024-01-20T18:09:00"},
        {"role": "manager", "text": "Да, при заказе от 1000 руб доставка бесплатно. Может добавите напитки или закуски?", "timestamp": "2024-01-20T18:10:00"},
        {"role": "client", "text": "Хорошо, добавьте колу 0.5л", "timestamp": "2024-01-20T18:11:00"},
        {"role": "manager", "text": "Отлично! Кола 0.5л - 120 руб. Итого 860 руб. Адрес доставки?", "timestamp": "2024-01-20T18:12:00"},
        {"role": "client", "text": "ул. Ленина, 10, кв. 5", "timestamp": "2024-01-20T18:13:00"},
        {"role": "manager", "text": "Спасибо! Заказ оформлен. Примерное время доставки 40 мин. Желаю приятного аппетита!", "timestamp": "2024-01-20T18:14:00"}
    ]
    
    metadata = {
        "manager": "Иван Петров",
        "channel": "whatsapp",
        "datetime": "2024-01-20 18:05",
        "funnel_stage": "горячий",
        "crm_link": "deal_12345",
        "chat_id": "test_chat_001"
    }
    
    result = analyze_chat_pro(test_chat, metadata)
    
    if result.get("error"):
        print(f"❌ Ошибка: {result.get('error_message')}")
    else:
        print(f"✅ Анализ успешен! Счет: {result.get('summary_score', 0)}/100")
        
        # Печатаем таблицу
        analyses_list = [result]
        print_pro_analysis_table(analyses_list)
        
        # Экспортируем в CSV
        export_to_csv(analyses_list, "test_okc_analysis.csv")
        
        print("\n✅ Тест завершен.")